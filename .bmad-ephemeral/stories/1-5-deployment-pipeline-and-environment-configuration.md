...
- [ ] Test pipeline by creating test PR (user will verify)
...
- [ ] **RESEARCH: Next.js 16 Dockerfile best practices**
  - [ ] Research Next.js standalone output mode configuration (next.config.ts)
  - [ ] Research official Next.js Docker examples (latest 2024-2025)
  - [ ] Research Node.js vs Bun as Docker base image (compatibility, size, performance)
  - [ ] Research production security hardening (non-root user, minimal base image)
  - [ ] Research health check endpoint implementation
  - [ ] Document recommended Dockerfile structure

- [ ] **RESEARCH: Multi-stage Docker builds**
  - [ ] Research build stage optimization (deps → build → production)
  - [ ] Research layer caching for faster builds
  - [ ] Research image size optimization techniques (Alpine vs Distroless)
  - [ ] Research .dockerignore best practices (what to exclude)
  - [ ] Research build args vs environment variables
  - [ ] Document multi-stage build strategy

- [ ] **RESEARCH: Docker registry integration with Dokploy**
  - [ ] Research if Dokploy has built-in Docker registry
  - [ ] Research using GitHub Container Registry (GHCR) with Dokploy
  - [ ] Research using Docker Hub with Dokploy
  - [ ] Research private registry options (Harbor, etc.) with Dokploy
  - [ ] Research authentication methods for each registry option
  - [ ] Research tagging strategies (latest, git SHA, semver, PR number)
  - [ ] Research registry storage limits and cleanup policies
  - [ ] Document recommended registry and tagging approach

- [ ] **RESEARCH: Docker layer caching in GitHub Actions**
  - [ ] Research docker/build-push-action v5+ features
  - [ ] Research BuildKit cache backends (inline, registry, gha)
  - [ ] Research cache size and performance tradeoffs
  - [ ] Research cache scoping (per-branch vs global)
  - [ ] Document recommended caching strategy

**⚠️ MANDATORY PAUSE POINT:**

- [ ] **STOP: Document and present Phase 2 research findings**
  - [ ] Create `.bmad-ephemeral/stories/1-5-research/phase-2-docker.md`
...
- [ ] (Tasks will be added here after user approves Phase 2 research findings)

**VERIFICATION:**

- [ ] Build Docker image locally - verify it works
- [ ] Run Docker container locally - verify app starts and works
- [ ] Verify health check endpoint responds correctly
- [ ] Create test PR - verify Docker build succeeds in CI
- [ ] Verify Docker image is pushed to registry with correct tags
- [ ] Verify image size is reasonable (< 500MB)
- [ ] Verify build caching works (second build should be faster)
...
**Files to Create (Tentative - may change based on research):**
...
- `.bmad-ephemeral/stories/1-5-research/phase-5-dependencies.md` - Phase 5 research (optional)
...
- `apps/web/src/lib/env.ts` - Environment variable validation (optional but recommended)
- `apps/web/src/app/api/health/route.ts` - Health check endpoint (if needed for Dokploy)

**Files to Verify/Update:**
- `turbo.json` - Verify task dependencies and caching
- `package.json` (root) - Verify build scripts
- `apps/web/package.json` - Verify build scripts, add standalone output
- `apps/web/next.config.ts` - Verify production optimizations, enable standalone output
...
- ✅ Configured caching: Bun dependencies via `actions/cache@v4` and Turborepo via `rharkor/caching-for-turbo@v1.5`
...
- ⏳ Pipeline ready for testing - user will create test PR to verify
...
- `.bmad-ephemeral/stories/1-5-research/phase-1-ci-pipeline.md` - Phase 1 research findings

**Modified:**
- `README.md` - Added CI/CD Pipeline section (lines 92-123)
